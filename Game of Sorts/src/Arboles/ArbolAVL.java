...
public class ArbolAVL {
    ...
    public void insert( Comparable t, int x, int y ){
        root = insert( t, root, x, y );
    }

    /*
     * x es una instancia de una clase que implementa Comparable
    */
    private AVLNode insert( Comparable M, AVLNode t, int x,int y ){
        if( t == null )
            t = new AVLNode( M, null, null, x, y );
        else if( M.compareTo( t.dato ) < 0 ) {
            t.izquierdo = insert( M, t.izquierdo, x, y );
            if( height( t.izquierdo ) - height( t.derecho ) == 2 )
                if( M.compareTo( t.izquierdo.dato ) < 0 )
                    t = rotateWithLeftChild( t ); /* Caso 1 */
                else
                    t = doubleWithLeftChild( t ); /* Caso 2 */
        }
        else if( M.compareTo( t.dato ) > 0 ) {
            t.derecho = insert( M, t.derecho, x, y );
            if( height( t.derecho ) - height( t.izquierdo ) == 2 )
                if( M.compareTo( t.derecho.dato ) > 0 )
    ...
    */
    ...
    //public void eliminar(Comparable M, AVLNode t, int x,int y){
    ...
    public AVLNode encuentraMin(AVLNode n) {
        if(n == null)
            return null;
        else if(n.izquierdo == null)
            return n;
        return encuentraMin(n.izquierdo);
    }

    /**
    El codigo anterior para eliminar cualquier duda de plagio fue tomado de 
    * la siguiente pagina:
    http://ciencias-y-sistemas.blogspot.com/2011/11/codigo-de-arbol-avl.html
     */
